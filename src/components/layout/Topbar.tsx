...
export function Topbar({ userName, expenseTotal = 0, incomeTotal = 0, balanceTotal = 0, month, year }: TopbarProps) {
  ...
  const renderMenuDropdown = () => {
    ...
    return (
      <>
        <div className="fixed inset-0 z-40 bg-black/1" onClick={() => setMenuOpen(false)} />
        ...
        <div className="px-4 py-2 border-b border-line mb-2 flex justify-between items-center">
          ...
          <button onClick={() => setMenuOpen(false)} className="text-muted hover:text-text p-1 -mr-2 bg-panel-soft rounded grid place-items-center">
      ...
      <div className="grid grid-cols-[52px_1fr_52px_52px] items-center pt-2 pb-2.5 px-4 relative group">
        ...
        {pathname.startsWith('/records') || pathname === '/' ? (
          <div className="relative">
            ...
            {filterOpen && (
              <>
                <div className="fixed inset-0 z-40 bg-black/1" onClick={() => setFilterOpen(false)} />
                <div className="absolute right-0 top-full mt-2 w-48 bg-panel border-2 border-line rounded-lg shadow-app py-2 z-50">
                <div className="px-4 pb-2 pt-1 border-b border-line mb-2 flex justify-between items-center">
                  <span className="text-[12px] font-black tracking-widest uppercase text-muted">Sort by</span>
                  <button onClick={() => setFilterOpen(false)} className="text-muted hover:text-text p-1 -mr-2 bg-panel-soft rounded grid place-items-center">
